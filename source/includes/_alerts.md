...
Librato provides many [notification services](https://kb-docs-archive.librato.com/docs/kb/alert/alerts-intro.html#notification-services) to alert your team by when an alert is triggered. At least one (but not lmiited to one) service must be tied to each alert.
