...
The general properties of a measurement are described below. The [documentation on submitting measurements](#submit-measurements) includes a full overview of all acceptable parameters.
...
>metric=cpu, {region=us-west,hostname=web-prod-3}
>metric=memory, {region=us-west,hostname=web-prod-3}
...
>This will result in the following data streams

```
metric=cpu, {region=us-west}
metric=memory, {region=us-east, db=db-prod-1, hostname=web-prod-3}
{
  "time": 1421530163,
  "tags": {
    "region": "us-west",
...
Parameter | Definition
--------- | ----------
2xx | Payload was parsed, but there may be have been internal errors (normally 200, but code should be able to accept a 202).
401 | Authorization error.
403 | Not permitted (read-only token, user blacklisted, account closed, etc)
415 | Content type was not application/json.
422 | Could not parse the basic format of the payload to identify measurements.
429 | Request was rate limited (it's possible some data was permitted, need to verify response body)
...
>Lookup by tags matching region=us* and hostname=*prod, grouped by region, and with grouping/summary functions.
...
summary_function | Defines the property of the metric to query. A metric has properties like: sum, count, min, max; this parameter defines which of the properties to read and represent in the payload. Acceptable values are: sum, min, max, count, mean, derivative, stddev. This parameter falls back to the persisted summary function of the metric, which by default is mean. The value derivative will return a derivative over the last value over every sample.

The summary function also defines the function used when downsampling data to a coarser granularity. The downsampling function is chosen as following given a summary function(SF):
...
* `region=us-east host=db-1-prod`
* `region=us-east host=db-2-prod`
* `region=us-west host=db-3-prod`
* `region=us-west host=db-stg`
* `region=us-east name=bubbles`
